...
def db1_df(lgn, month, year):
	...
	colset = ['ItemName', 'ItemPlatform', 'ItemPublisher', 'ItemDataType', 'Category', 'Date', 'MetricType', 'Count']
	df = pd.DataFrame(columns = colset)
	try:
...
def br2_df(lgn, month, year):
	...
	colset = [	'ItemName', 'ItemPlatform', 
				'ItemPublisher', 'ItemDataType', 
				'Print_ISBN', 'Online_ISBN', 
				'DOI', 'Category', 
				'Date', 'MetricType', 
				'Count']
	df = pd.DataFrame(columns = colset)
